...
spot_simulate <- function(dimension = c(78, 64), n = 5000, 
  ...
  if (structure == "interlaced"){
    ...
    y_err <- runif(dimension[1] * dimension[2], -err_p, err_p) * sqrt(3) / 2
  ...
  spatial_info[,1] <- spatial_info[,1] * (limitx[2] - limitx[1]) + limitx[1]
  spatial_info[,2] <- spatial_info[,2] * (limity[2] - limity[1]) + limity[1]
